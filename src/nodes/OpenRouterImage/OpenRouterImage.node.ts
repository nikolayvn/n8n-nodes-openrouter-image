...
import { nodeProperties } from './OpenRouterImageDescription';
...
export class OpenRouterImage implements INodeType {
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
        ...
        try {
            ...
            // Проверяем API key
            if (!apiKey) {
                throw new NodeOperationError(this.getNode(), 'API Key не указан');
            }
            
            // Получаем параметры
            const model = this.getNodeParameter('ai_model', itemIndex, 'google/gemini-2.5-flash-image-preview') as string;
            const user_message = this.getNodeParameter('user_message', itemIndex, '') as string;
            const aspect_ratio = this.getNodeParameter('options.aspect_ratio', itemIndex, '1:1') as string;
            const inputPhotos = this.getNodeParameter('options.inputPhotos', itemIndex, []) as string[];
            ...
            const user_contents: Array<{type: string; text?: string, image_url?:object }> = [];
            ...
            if (messages.length === 0) {
                throw new NodeOperationError(this.getNode(), 'Either user_message or system_message is required');
            }
            ...
            if (response.choices && response.choices.length > 0) {
                ...
                // Вариант 2: изображения в message.content (массив)
                    ...
                    images = message.content.filter((item: any) => 
                        item.type === 'image' && item.image_url
                    ).map((item: any) => ({ image_url: item.image_url }));
                }
                ...
                for (let imgIndex = 0; imgIndex < images.length; imgIndex++) {
                    ...
                    try {
                        ...
                        const binaryData = await this.helpers.prepareBinaryData(
                            imageBuffer,
                            `generated-${Date.now()}-${imgIndex + 1}.${getExtension(mimeType)}`,
                            mimeType
                        );
                ...
                throw new NodeOperationError(
                    this.getNode(),
                    'Нет ответа от модели или он пустой',
                    { itemIndex }
                );
            }
            ...
            if (this.continueOnFail()) {
                ...
                throw new NodeOperationError(this.getNode(), error.message || String(error), { itemIndex });
            }
        }
